...
- **2026-05-04** — Codex Quick Reads FAQ accordion shipped on Ask Anything page. 20 curated Q&As in three groups: Urvil's questions (7), Veronika's questions (6), Us (7). Static, hand-vetted, no LLM calls. Native HTML <details>/<summary> elements for tap-to-expand, scoped CSS via .ask-faq-* classes. Lives in the {!answer && (...)} branch of AskPage, after the existing starters section. Maintained via Python find-replace when blueprint facts change. SHIP NOTES: 7 false starts during this session traced to two root causes — (1) JSX inside {} braces broke when using arrow-with-paren-wrap (=> (<JSX>)); v6 fix uses function() { return <JSX>; } syntax. (2) CSS injection using rfind('</style>') matched a literal </style> string inside the JS Babel bundle, dumping CSS into JavaScript and breaking parsing; v7 fix uses find('</style>') (first occurrence, the actual page CSS closing tag). Both pitfalls now documented in case future blueprint expansions need similar JSX or CSS injections. Commit: f4cb45c.
